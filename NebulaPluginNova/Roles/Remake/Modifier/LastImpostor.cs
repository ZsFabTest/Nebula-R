...
static file class LIGuesserSystem
{
    ...
    static private SpriteLoader targetSprite = SpriteLoader.FromResource("Nebula.Resources.Remake.LITargetIcon.png", 115f);
...
public class LISpawnPatch
{
    public static void Postfix(PlayerControl __instance)
    {
        if (NebulaGameManager.Instance == null || NebulaGameManager.Instance.LocalPlayerInfo == null) return;
        if (!LastImpostor.CanSpawnOption) return;
        if (NebulaGameManager.Instance.AllPlayerInfo.Count(p => p.IsImpostor) <= 1 && 
            NebulaGameManager.Instance.LocalPlayerInfo.IsImpostor && 
            !NebulaGameManager.Instance.LocalPlayerInfo.TryGetModifier<LastImpostor.Instance>(out _))
        {
            NebulaGameManager.Instance.LocalPlayerInfo.Unbox().RpcInvokerSetModifier(LastImpostor.MyRole, null).InvokeSingle();
        }
    }
}