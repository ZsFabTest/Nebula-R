...
public class Yandere : DefinedRoleTemplate, HasCitation, DefinedRole
{
    ...
    public class Instance : RuntimeAssignableTemplate, RuntimeRole
    {
        ...
        {
            Target = Helpers.GetPlayer((byte)arguments.Get(0, System.Random.Shared.Next(0, NebulaGameManager.Instance?.AllPlayersNum ?? 0))).GetModInfo();
        }
        ...
        [OnlyMyPlayer]
        void CheckExtraWin(PlayerCheckExtraWinEvent ev) => ev.SetWin(ev.WinnersMask.Test(Target) & ev.GameEnd == NebulaGameEnd.YandereWin);
        ...
        void RuntimeAssignable.OnActivated()
        {
            if (AmOwner)
            {
                ...
                var myTracker = Bind(ObjectTrackers.ForPlayer(null, MyPlayer, p => !p.IsDead && p.PlayerId != Target!.PlayerId && players[p.PlayerId] >= 5f));
                ...
                killButton.OnClick = (button) =>
                {
                    ...
                    new StaticAchievementToken("yandere.common");
        ...
        void Update(GameUpdateEvent ev)
        {
            ...
            else if(!(Target?.IsDead ?? true) && !MyPlayer.IsDead && NebulaGameManager.Instance?.AllPlayerInfo().Count(p => !p.IsDead) <= 4)
                NebulaGameManager.Instance?.RpcInvokeSpecialWin(NebulaGameEnd.VultureWin, 1 << Target.PlayerId);
