...
public class Bomb : INebulaScriptComponent
{
    ...
    public bool OnActivated(Virial.Game.Player player, Vent vent, ref int killNum)
    {
        ...
        {
            Helpers.GetPlayer(ownerId).GetModInfo()?.MurderPlayer(player, RemakeInit.PlayerStatus.Exploded, null, KillParameter.RemoteKill);
            this.ReleaseIt();
            return true;
        }
        return false;
...
public class Miner : DefinedRoleTemplate, HasCitation, DefinedRole
{
    ...
    public class Instance : RuntimeAssignableTemplate, RuntimeRole
    {
        ...
        // int[]? RuntimeAssignable.RoleArguments => ventId.ToArray();
        private static Image buttonSprite = SpriteLoader.FromResource("Nebula.Resources.Remake.LayingMinesButton.png", 115f);
        List<Bomb> bombs = new();
        ...
        void RuntimeAssignable.OnActivated()
        {
            if (AmOwner)
            {
                ...
                button.ShowUsesIcon(3).text = mineNum.ToString();
                button.CoolDownTimer = Bind(new Timer(LayingMineCoolDownOption).SetAsAbilityCoolDown().Start());
                button.SetLabelType(Virial.Components.ModAbilityButton.LabelType.Standard);
                button.SetLabel("layMines");
            }
        }
